...
const izumi = usync_1.makeUSyncSocket(config)
const { ev, ws, authState, generateMessageTag, sendNode, query, onUnexpectedError, groupFetchAllParticipating } = izumi
let privacySettings
...
const result = await izumi.executeUSyncQuery(usyncQuery)
if (result) {
return result.list
}
}
...
const results = await izumi.executeUSyncQuery(usyncQuery)
if (results) {
