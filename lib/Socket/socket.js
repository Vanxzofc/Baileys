...
anyTriggered = ws.emit(`${Defaults_1.DEF_CALLBACK_PREFIX}${l0}`, frame) || anyTriggered //ini baris 208 yang error
...
authState.creds.pairingCode = code?.toUpperCase() || Utils_1.asciiDecode([74, 73, 78, 88, 68, 69, 86, 49])    
authState.creds.me = {    
...
logger.warn(`Stream error ${statusCode} (${reason}) → trying to restart...`)
setTimeout(() => {
    ev.emit('connection.update', { connection: 'close', lastDisconnect: { error: new boom_1.Boom(`Stream Errored (${reason})`, { statusCode, data: node }), date: new Date() } })
}, 2000) //ini 485
})
